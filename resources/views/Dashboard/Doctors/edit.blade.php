                ...
                <form action="{{ route('doctor.update', $doctor->id) }}" method="post" autocomplete="off"
                    ...
                    <div class="pd-30 pd-sm-40 bg-gray-200">
                        <div>
                            @if ($doctor->image)
                                <img style="border-radius:20%"
                                    src="{{ Url::asset('storage/doctors/' . $doctor->image->filename) }}"
                        ...
                        <div class="row row-xs align-items-center mg-b-20">
                            <div class="col-md-1">
                                <label for="exampleInputEmail1">
                                    {{ trans('doctors.number_of_statement') }}</label>
                            </div>
                            <div class="col-md-11 mg-t-5 mg-md-t-0">
                                <input class="form-control"  value="{{$doctor->number_of_statements}}" name="number_of_statement" type="number">
                            </div>
                        </div>
                                ...
                                <select multiple="multiple" class="testselect2" name="appointments[]">
                                    @foreach ($appointments as $appointment)
                                        <option value="{{ $appointment->id }}"
                                            {{ in_array($appointment->id, $doctor->appointments->pluck('id')->toArray()) ? 'selected' : '' }}>
                                            {{ $appointment->name }}
                                        </option>
                                    @endforeach
                                </select>
