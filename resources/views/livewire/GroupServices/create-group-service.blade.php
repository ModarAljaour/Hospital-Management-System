<div>
    ...
    @if ($ServiceSaved)
        <div class="alert alert-info">{{ trans('group_service.message_true') }}</div>
    @endif

    @if ($ServiceUpdated)
        <div class="alert alert-info">{{ trans('group_service.message_update') }}</div>
    @endif
    @if ($show_table)
                ...
                <div class="card-header">
                    <div class="col-md-12">
                        <button class="btn btn-outline-primary"
                            wire:click.prevent="addService">{{ trans('group_service.add_Service') }}
                        </button>
                    </div>
                </div>
                ...
                <div class="card-body">
                        ...
                        <table class="table table-bordered">
                            <thead>
                                <tr class="table-primary">
                                    <th>{{ trans('group_service.service_name') }}</th>
                                    <th width="200">{{ trans('group_service.number') }}</th>
                                    <th width="200">{{ trans('group_service.operation') }}</th>
                                </tr>
                            </thead>
                            <tbody>
                    ...
                    <div class="col-lg-4 ml-auto text-right">
                        <table class="table pull-right">
                            <tr>
                                <td style="color: red">{{ trans('group_service.total') }}</td>
                                <td>{{ number_format($subtotal, 2) }}</td>
                            </tr>

                            <tr>
                                <td style="color: red">{{ trans('group_service.discount_value') }}</td>
                                <td width="125">
                            ...
                            <tr>
                                <td style="color: red">{{ trans('group_service.tax_percent') }}</td>
                                <td>
                            ...
                            <tr>
                                <td style="color: red">{{ trans('group_service.total_with_tax') }}</td>
                    ...
                    <div>
                        <input class="btn btn-outline-success" type="submit"
                            value="{{ trans('group_service.submit_data') }}">
                    </div>
                </div>
            </div>
