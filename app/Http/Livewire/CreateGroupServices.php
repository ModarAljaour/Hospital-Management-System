...
class CreateGroupServices extends Component
{
    ...
    public function render()
    {
        ...
        return view('livewire.GroupServices.create-group-service', [
            'groups' => Group::all(),
    ...
    public function addService()
    {
        foreach ($this->GroupsItems as $key => $groupItem) {
            if (!$groupItem['is_saved']) {
                $this->addError('GroupsItems.' . $key, trans('group_service.line_save'));
                return;
            }
        }
    ...
    public function editService($index)
    {
        foreach ($this->GroupsItems as $key => $groupItem) {
            if (!$groupItem['is_saved']) {
                $this->addError('GroupsItems.' . $key, trans('group_service.line_edit'));
                return;
            }
        }
    ...
    {
        Group::destroy($id);
        return redirect()->to('/add_group_service');
    }
}
